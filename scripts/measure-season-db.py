"""Replay the synthetic P3 year through the real SQL RPC in disposable PostgreSQL."""
from pathlib import Path
import json,re,subprocess,time,uuid
root=Path(__file__).resolve().parent.parent
name='verdant-p3-size-'+uuid.uuid4().hex[:8]
...
try:
 ...
 patches=[json.loads(line) for line in (root/'artifacts/p3-year-patches.ndjson').read_text().splitlines()]
 def literal(value):
 ...
 out={'kind':'PostgreSQL 17 in local Docker, actual one-year delta RPCs, three retained checkpoints; excludes Supabase platform/Auth overhead','commands':len(patches),'writeP95Ms':times[int(len(times)*.95)],'loadMs':float(re.findall(r'Time: ([\d.]+) ms',read)[0]),'allocatedBytes':allocated,'allocatedTotalBytes':sum(allocated.values()),'afterVacuumFullBytes':compacted,'afterVacuumFullTotalBytes':sum(compacted.values())}
 (root/'artifacts/p3-database-metrics.json').write_text(json.dumps(out,indent=2));print(json.dumps(out,indent=2))
