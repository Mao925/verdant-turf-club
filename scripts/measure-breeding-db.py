...
name='verdant-p5-size-'+uuid.uuid4().hex[:8]
...
try:
 ...
 patches=[json.loads(line) for line in (root/'artifacts/p5-five-year-patches.ndjson').read_text().splitlines()]
 def literal(value):
 ...
 out={'kind':'PostgreSQL 17 in local Docker, actual five-year delta RPCs, three retained checkpoints; excludes Supabase platform/Auth overhead','commands':len(patches),'writeP95Ms':times[int(len(times)*.95)],'loadMs':float(re.findall(r'Time: ([\d.]+) ms',read)[0]),'allocatedBytes':allocated,'allocatedTotalBytes':sum(allocated.values()),'afterVacuumFullBytes':compacted,'afterVacuumFullTotalBytes':sum(compacted.values())}
 (root/'artifacts/p5-database-metrics.json').write_text(json.dumps(out,indent=2));print(json.dumps(out,indent=2))
