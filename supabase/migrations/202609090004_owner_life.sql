-- P4 health, placement and person memory records. Existing versions, CAS and retry fingerprints remain compatible.
begin;
...
begin
  ...
  for item in select value from jsonb_array_elements(p_upserts) loop
    if jsonb_typeof(item)<>'object' or item->>'id' is null or (item->>'id')!~'^[a-zA-Z0-9:_-]{1,150}$' or (item->>'id') in ('__proto__','constructor','prototype') or (item->>'kind') not in ('horse','contract','ledger','event','market','invoice','consultation','race','npc-owner','health','placement','scene') or item->>'kind' is null then raise exception 'invalid entity' using errcode='22023'; end if;
