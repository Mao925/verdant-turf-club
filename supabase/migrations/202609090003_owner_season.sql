-- P3 version and persistent NPC-owner entities; P1/P2 retry fingerprints and CAS stay compatible.
begin;
...
begin
  ...
  if (p_core->>'saveId') is distinct from p_save_id::text or (p_core->>'schemaVersion') is distinct from '1' or not (((p_core->>'engineVersion') is not distinct from 'owner-p1' and (p_core->>'rulesetVersion') is not distinct from 'foundation-2026') or ((p_core->>'engineVersion') is not distinct from 'owner-p2' and (p_core->>'rulesetVersion') is not distinct from 'prototype-2026') or ((p_core->>'engineVersion') is not distinct from 'owner-p3' and (p_core->>'rulesetVersion') is not distinct from 'calendar-2026')) then raise exception 'unsupported save version' using errcode='22023'; end if;
  ...
  for item in select value from jsonb_array_elements(p_upserts) loop
    if jsonb_typeof(item)<>'object' or item->>'id' is null or (item->>'id')!~'^[a-zA-Z0-9:_-]{1,150}$' or (item->>'id') in ('__proto__','constructor','prototype') or (item->>'kind') not in ('horse','contract','ledger','event','market','invoice','consultation','race','npc-owner') or item->>'kind' is null then raise exception 'invalid entity' using errcode='22023'; end if;
