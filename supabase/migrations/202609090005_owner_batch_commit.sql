-- Bulk insert replaces per-entity SQL calls after a real P4 three-year restore hit statement_timeout.
-- Keep the canonical fingerprint, transaction, checkpoint and compare-and-swap contract unchanged.
begin;
...
begin
  ...
  if (p_core->>'saveId') is distinct from p_save_id::text or (p_core->>'schemaVersion') is distinct from '1' or not (((p_core->>'engineVersion') is not distinct from 'owner-p1' and (p_core->>'rulesetVersion') is not distinct from 'foundation-2026') or ((p_core->>'engineVersion') is not distinct from 'owner-p2' and (p_core->>'rulesetVersion') is not distinct from 'prototype-2026') or ((p_core->>'engineVersion') is not distinct from 'owner-p3' and (p_core->>'rulesetVersion') is not distinct from 'calendar-2026') or ((p_core->>'engineVersion') is not distinct from 'owner-p4' and (p_core->>'rulesetVersion') is not distinct from 'life-2026')) then raise exception 'unsupported save version' using errcode='22023'; end if;
  ...
  if exists (
    ...
    where jsonb_typeof(item)<>'object' or item->>'id' is null
      ...
      or (item->>'kind') not in ('horse','contract','ledger','event','market','invoice','consultation','race','npc-owner','health','placement','scene')
  ...
  insert into public.owner_entities(save_id,id,body)
    ...
    on conflict(save_id,id) do update set body=excluded.body;
